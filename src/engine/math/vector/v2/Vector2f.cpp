...
float jgl::Vector2f::dst(jgl::Vector2f* vec2f){
    float deltaX = this->x-vec2f->x;
    float deltaY = this->y-vec2f->y;
    return sqrt( pow(deltaX, 2) + pow(deltaY, 2));
}



float jgl::Vector2f::lenght(){
...
float jgl::Vector2f::dot(jgl::Vector2f* vec2f){
    return this->x*vec2f->x + this->y*vec2f->y;
}



float jgl::Vector2f::crs(jgl::Vector2f* vec2f){
    return  this->x*vec2f->y - this->y*vec2f->x;
}



float jgl::Vector2f::angleRad(jgl::Vector2f* vec2f){
    float norm = this->lenght();
    float ps = this->dot(vec2f);
    float normParam = vec2f->lenght();
    return acos(ps/(norm*normParam));
}



float jgl::Vector2f::angleGrd(jgl::Vector2f* vec2f){
...
bool jgl::Vector2f::oppositeDirection(jgl::Vector2f* vec2f){
    ...
    isNegative(this->x) != isNegative((float)vec2f->x) &&
    isNegative(this->y) != isNegative((float)vec2f->y);
    return flag;
}
...
jgl::Vector2f* jgl::Vector2f::rotate(float _rad){
...
jgl::Vector2f* jgl::Vector2f::rotateAround(jgl::Vector2f* vec2f, float _rad){
...
jgl::Matrix3* jgl::Vector2f::getRotationMatrix(){
    return new jgl::Matrix3(this->angleRad(&jgl::Vector2f::xAxis));
}
jgl::Matrix3* jgl::Vector2f::getTranslationMatrix(){
    return (new jgl::Matrix3())->setToTranslation(this->x, this->y);
}
jgl::Matrix3* jgl::Vector2f::getScalingMatrix(){
