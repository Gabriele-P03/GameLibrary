#include "Vector3d.h"
 ...
 jgl::Vector3d jgl::Vector3d::xAxis = jgl::Vector3d(1.0, 0.0, 0.0);
 jgl::Vector3d jgl::Vector3d::yAxis = jgl::Vector3d(0.0, 1.0, 0.0);
 jgl::Vector3d jgl::Vector3d::zAxis = jgl::Vector3d(0.0, 0.0, 1.0);
...
jgl::Vector3d::Vector3d(std::string* fromString){
    ...
    this->x = atof(fromString->substr(0, firstIndex).c_str());
    this->y = atof(fromString->substr(firstIndex+1, lastIndex).c_str());
    this->z = atof(fromString->substr(lastIndex+1, fromString->size()).c_str());
}
...
void jgl::Vector3d::setX(double x){this->x = x;}
void jgl::Vector3d::setY(double y){this->y = y;}
void jgl::Vector3d::setZ(double z){this->z = z;}
void jgl::Vector3d::setAll(double x, double y, double z){this->x = x; this->y = y; this->z = z;}
void jgl::Vector3d::set(Vector3d* vec3d){this->setAll( (*vec3d).x, (*vec3d).y, (*vec3d).z );}

void jgl::Vector3d::addX(double x){this->x += x;}
void jgl::Vector3d::addY(double y){this-> y += y;}
void jgl::Vector3d::addZ(double z){this-> z += z;}

void jgl::Vector3d::add(Vector3d* vec3d){
    this->addX( (*vec3d).x ); 
    this->addY( (*vec3d).y );
    this->addZ( (*vec3d).z );
}

void jgl::Vector3d::addAll(double x, double y, double z){
    this->addX(x);
    this->addY(y);
    this->addZ(z);
}

void jgl::Vector3d::mulX(double x){this->x *= x;}
void jgl::Vector3d::mulY(double y){this->y *= y;}
void jgl::Vector3d::mulZ(double z){this->z *= z;}
void jgl::Vector3d::mul(Vector3d* vec3d){
    this->mulX( (*vec3d).x );
    this->mulY( (*vec3d).y );
    this->mulZ( (*vec3d).z );
}
void jgl::Vector3d::mulAll(double x, double y, double z){this->mulX(x); this->mulY(y); this->mulZ(z);}

jgl::Vector3d* jgl::Vector3d::cpy(){return new jgl::Vector3d(this->x, this->y, this->z);}
...
void jgl::Vector3d::rotate(double _rad, jgl::Vector3d* axis){
...
void jgl::Vector3d::rotateAround(jgl::Vector3d* vec3d, double _rad, jgl::Vector3d* axis){
