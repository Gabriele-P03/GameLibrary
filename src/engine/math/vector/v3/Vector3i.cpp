...
void jgl::Vector3i::setX(int x){this->x = x;}
void jgl::Vector3i::setY(int y){this->y = y;}
void jgl::Vector3i::setZ(int z){this->z = z;}
void jgl::Vector3i::setAll(int x, int y, int z){this->x = x; this->y = y; this->z = z;}
void jgl::Vector3i::set(Vector3i* vec3i){this->setAll( (*vec3i).x, (*vec3i).y, (*vec3i).z );}

void jgl::Vector3i::addX(int x){this->x += x;}
void jgl::Vector3i::addY(int y){this-> y += y;}
void jgl::Vector3i::addZ(int z){this-> z += z;}

void jgl::Vector3i::add(Vector3i* vec3i){
    this->addX( (*vec3i).x ); 
    this->addY( (*vec3i).y );
    this->addZ( (*vec3i).z );
}

void jgl::Vector3i::addAll(int x, int y, int z){
    this->addX(x);
    this->addY(y);
    this->addZ(z);
}

void jgl::Vector3i::mulX(int x){this->x *= x;}
void jgl::Vector3i::mulY(int y){this->y *= y;}
void jgl::Vector3i::mulZ(int z){this->z *= z;}
void jgl::Vector3i::mul(Vector3i* vec3i){
    this->mulX( (*vec3i).x );
    this->mulY( (*vec3i).y );
    this->mulZ( (*vec3i).z );
}
void jgl::Vector3i::mulAll(int x, int y, int z){this->mulX(x); this->mulY(y); this->mulZ(z);}

jgl::Vector3i* jgl::Vector3i::cpy(){return new jgl::Vector3i(this->x, this->y, this->z);}
...
void jgl::Vector3i::rotate(double _rad, jgl::Vector3i* axis){
...
void jgl::Vector3i::rotateAround(jgl::Vector3i* vec3i, double _rad, jgl::Vector3i* axis){
